...
水位参数由 DRAM 容量和工作负载控制参数动态计算。实现首先根据 $\alpha$ 计算目标预留页框数，再通过 $\beta$ 得到低水位，并约束 $1\leq F_{\mathrm{low}}<F_{\mathrm{target}}<D$ 以及目标预留比例上限。当前正式配置采用 $\alpha=0.15$、$\beta=0.4$，使不同容量和工作负载的水位参数按照同一规则派生。
